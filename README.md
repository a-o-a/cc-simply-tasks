...
| Drizzle ORM | 0.44.x | Prisma 엔진 다운로드 없이 npm 기반으로 동작 |
...
>
> Prisma 제거와 libsql 전환 결과 메모는 [`docs/DRIZZLE_MIGRATION.md`](./docs/DRIZZLE_MIGRATION.md)를 참고.
...
- 현재 런타임은 `@libsql/client` 기반이라 Prisma처럼 별도 엔진 바이너리를 내려받지 않습니다.
...
| `npm run db:migrate` | 생성된 migration 적용 | 있음 |
