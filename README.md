...
내부용 작업(업무) 관리 시스템. Next.js 13.5 (app router) + Drizzle ORM + SQLite 기반이며, 이후 Postgres 이관을 염두에 둔 구조로 설계됨.
...
| Drizzle ORM | 0.44.x | Rust 엔진 다운로드 없이 npm 기반으로 동작 |
...
│   ├── db.ts                    # Drizzle DB 초기화 + SQLite PRAGMA
│   ├── db/
...
>
> Prisma 내부망 이슈 대응과 Drizzle 전환 검토 메모는 [`docs/DRIZZLE_MIGRATION.md`](./docs/DRIZZLE_MIGRATION.md)를 참고.
